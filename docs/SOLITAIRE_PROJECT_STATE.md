...
- Main 10 buyer payload source-availability checkpoint: payload capture status is separated from source availability. Engine state, ruleset/draw mode, source listing, listing value, and schema version show source availability `Available`; resume point and ownership target remain `Not available`. The approved buyer-copy envelope schema identifier is `buyer-game-copy-v1`, while the captured engine state retains its own engine version `1.0.0`. All seven payload components remain `Not captured`, so payload readiness remains `No`, combined buyer game-copy readiness remains `No`, `COPY_BLOCKER_PAYLOAD_NOT_READY` remains the only buyer game-copy blocking condition, and Preview Purchase remains disabled. The readiness message says `Locked — payload requirements not complete` because the five source requirements are complete.
