...
- Main 10 buyer game-copy payload contract and builder checkpoint: dedicated module `packages/web/src/buyerGameCopy.ts` exports shared constants `BUYER_GAME_COPY_SCHEMA_VERSION`, `BUYER_GAME_COPY_RESUME_RULE`, `BUYER_GAME_COPY_RESUME_POINT`, and `BUYER_GAME_COPY_OWNERSHIP_TARGET_RULE`, source contract `BuyerGameCopySourceSnapshot`, payload contract `BuyerGameCopyPayloadV1`, and pure helper `buildBuyerGameCopyPayloadV1()`. The payload contract defines exactly seven required top-level fields: `engineState`, `rulesetDrawMode`, `sourceListing`, `listingValue`, `resumePoint`, `ownershipTarget`, and `schemaVersion`. The live `PreviewOnlyListingSnapshot` type extends `BuyerGameCopySourceSnapshot`, keeping its six payload-source fields synchronized with the builder while retaining its separate preview-only display fields. The live preview, payload type, and builder now share the same `buyer-game-copy-v1` schema identifier, exact frozen-position resume rule, structured empty-history/zero-undos resume point, and authenticated settled-buyer ownership-target rule. The helper deep-clones the frozen listing engine state, preserves the exact board, score, tick, ruleset, and draw mode, clears buyer undo history, resets buyer undos to `0`, attaches the authenticated buyer account ID, and leaves the source snapshot unchanged. Focused tests in `packages/web/tests/buyerGameCopy.test.ts` pass `2 / 2`. The live app imports the shared constants and source type but does not import or call the builder, so no runtime buyer payload, buyer identity, component capture, purchase execution, ownership transfer, persistence, or playable handoff has been created or enabled; payload readiness remains `0 / 7`, and Preview Purchase remains disabled. Web production build and lint pass.
