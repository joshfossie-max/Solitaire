...
- Preview Listing Detail now includes a Preview purchase guard summary showing guard active `Yes`, handoff requirements complete `No`, purchase button state `Disabled`, disabled reason code `HANDOFF_REQUIREMENTS_INCOMPLETE`, and execution mode `Preview only`.
