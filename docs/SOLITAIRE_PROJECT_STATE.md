...
- Main 10 buyer payload source-availability checkpoint: payload capture status is now separated from source availability. Engine state, ruleset/draw mode, source listing, and listing value show source availability `Available`; resume point, ownership target, and schema version show `Not available`. All seven payload components remain `Not captured`, so payload readiness remains `No`, combined buyer game-copy readiness remains `No`, `COPY_BLOCKER_PAYLOAD_NOT_READY` remains the only buyer game-copy blocking condition, and Preview Purchase remains disabled. The readiness message now correctly says `Locked — payload requirements not complete` because the five source requirements are complete.
