...
- Main 10 typed buyer game-copy payload contract checkpoint: exported TypeScript type `BuyerGameCopyPayloadV1` now defines exactly seven required top-level fields: `engineState`, `rulesetDrawMode`, `sourceListing`, `listingValue`, `resumePoint`, `ownershipTarget`, and `schemaVersion`. The contract fixes the ruleset to `classic_v1`, the envelope schema to `buyer-game-copy-v1`, and the approved resume behavior to an exact frozen listing position with empty buyer undo history and buyer undos starting at `0`. This is a type-only contract; no payload instance, buyer identity, component capture, purchase execution, ownership transfer, persistence, or playable handoff has been created or enabled. Web production build and lint pass.
