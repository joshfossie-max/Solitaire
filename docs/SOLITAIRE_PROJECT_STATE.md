...
- Preview buyer game copy shows buyer game copy `Not created`, buyer resume point `Exact frozen listing position; buyer undo history starts empty and buyer undos start at 0`, buyer playable state `Not enabled`, required before playable `Captured buyer game-copy payload and ownership rules`, and copy status `Preview only — buyer cannot resume or play this listing`.
- Main 10 buyer game-copy source checkpoint: Preview Listing Detail distinguishes listing metadata, the captured playable game-state snapshot, the identified buyer-copy source artifact, the listing-time freeze rule, and the approved post-settlement copy-creation trigger. All five source requirements are now complete, derived source readiness shows `Yes`, and the Preview handoff plan summary reads from that gate while buyer-copy creation remains disabled.
- Main 10 buyer game-copy source traceability checkpoint: five structured source requirements cover the listing metadata snapshot, playable game-state snapshot, buyer copy source, source freeze rule, and copy creation trigger; derived counts now show `5` total, `5` complete, and `0` incomplete. The source blocking-requirement list is empty, `COPY_BLOCKER_SOURCE_NOT_READY` is inactive, and only payload-related blockers continue through the buyer game-copy readiness summary, Preview purchase guard summary, top-level Buyer handoff readiness model, and Preview handoff plan summary while all copy creation, purchase, ownership-transfer, persistence, and playable-handoff behavior remains disabled and preview-only.
- Main 10 playable source snapshot checkpoint: preview listing creation captures a fully detached seller engine-state snapshot through the tested `cloneEngineState()` helper, including nested arrays, score breakdown, and undo history. The captured preview listing engine-state snapshot is identified as the future buyer-copy source artifact and remains frozen at preview listing creation. The approved future copy-creation trigger is `After successful purchase settlement and ownership transfer confirmation`; preview listing creation, viewing, and locked attempts do not create a buyer copy. All `5` buyer game-copy source requirements are now complete and source readiness shows `Yes`. The separate seven-component buyer payload remains `0` captured / `7` missing, combined buyer game-copy readiness remains `No`, `COPY_BLOCKER_PAYLOAD_NOT_READY` remains active, and Preview Purchase remains disabled; no buyer payload, ownership transfer, persistence, purchase execution, or playable handoff has been enabled. Engine build, web production build and lint pass, with `22` test files / `56` tests passing and coverage at statements `97.05%`, branches `88.29%`, functions `93.93%`, and lines `97.05%`.
...
- Main 10 buyer game-copy payload contract and builder checkpoint: dedicated module `packages/web/src/buyerGameCopy.ts` exports shared constants `BUYER_GAME_COPY_SCHEMA_VERSION`, `BUYER_GAME_COPY_RESUME_RULE`, `BUYER_GAME_COPY_RESUME_POINT`, `BUYER_GAME_COPY_OWNERSHIP_TARGET_RULE`, and `BUYER_GAME_COPY_CREATION_TRIGGER_RULE`, source contract `BuyerGameCopySourceSnapshot`, creation-confirmation contract `BuyerGameCopyCreationConfirmation`, payload contract `BuyerGameCopyPayloadV1`, and pure helper `buildBuyerGameCopyPayloadV1()`. The payload contract defines exactly seven required top-level fields: `engineState`, `rulesetDrawMode`, `sourceListing`, `listingValue`, `resumePoint`, `ownershipTarget`, and `schemaVersion`. The live `PreviewOnlyListingSnapshot` type extends `BuyerGameCopySourceSnapshot`, keeping its six payload-source fields synchronized with the builder while retaining its separate preview-only display fields. The live preview, payload type, and builder share the same `buyer-game-copy-v1` schema identifier, exact frozen-position resume rule, structured empty-history/zero-undos resume point, authenticated settled-buyer ownership-target rule, and post-settlement copy-creation trigger `After successful purchase settlement and ownership transfer confirmation`. `BuyerGameCopyCreationConfirmation` carries runtime boolean settlement and ownership-transfer confirmations, and `buildBuyerGameCopyPayloadV1()` throws unless both are `true`, preventing payload construction before both approved trigger conditions are satisfied. The builder rejects an empty or whitespace-only `authenticatedBuyerAccountId` and trims surrounding whitespace before storing the authenticated buyer account ID in the payload. The helper deep-clones the frozen listing engine state, preserves the exact board, score, tick, ruleset, and draw mode, clears buyer undo history, resets buyer undos to `0`, attaches the normalized authenticated buyer account ID, and leaves the source snapshot unchanged. Focused tests in `packages/web/tests/buyerGameCopy.test.ts` pass `6 / 6`, including explicit rejection before settlement confirmation, rejection before ownership-transfer confirmation, rejection without an authenticated buyer account ID, and normalization of surrounding buyer-ID whitespace. The live app imports the shared constants and source type but does not import or call the builder, so no runtime buyer payload, buyer identity, component capture, purchase execution, ownership transfer, persistence, or playable handoff has been created or enabled; payload readiness remains `0 / 7`, and Preview Purchase remains disabled. Web production build and lint pass.
- Main 10 buyer payload source-availability checkpoint: payload capture status is separated from source availability. Engine state, ruleset/draw mode, source listing, listing value, resume point, and schema version show source availability `Available`; the actual ownership target remains `Not available` because no authenticated buyer or successfully settled purchase exists. The engine-state source preview identifies the `Frozen preview listing engine-state snapshot` and shows `Not available` before that snapshot exists; the ruleset/draw-mode source preview derives from the frozen listing engine-state snapshot rather than the current live-game draw mode; the buyer-handoff source listing derives from the frozen snapshot’s `listingIdLabel` rather than being rebuilt from the current live-game seed; and the payload listing-value preview derives only from the frozen snapshot’s `currentListingValueLabel` rather than falling back to the current live listing draft. These previews therefore continue to describe the listed game even if the live game later changes. The approved ownership-target rule is `Authenticated buyer account from the successfully settled purchase; assigned only after ownership transfer confirmation`, the approved resume rule is `Exact frozen listing position; buyer undo history starts empty and buyer undos start at 0`, the buyer-copy envelope schema identifier is `buyer-game-copy-v1`, and the captured engine state retains its own engine version `1.0.0`. All seven payload components remain `Not captured`, so payload readiness remains `No`, combined buyer game-copy readiness remains `No`, `COPY_BLOCKER_PAYLOAD_NOT_READY` remains the only buyer game-copy blocking condition, and Preview Purchase remains disabled. The readiness message says `Locked — payload requirements not complete` because the five source requirements are complete.
- Main 10 preview ownership-transfer policy checkpoint: the current owner remains the original player until successful purchase settlement is confirmed. At settlement confirmation, ownership would transfer atomically to the authenticated buyer account, the listing would be marked sold, and the seller would lose the ability to resume, modify, or relist the sold game. Listing creation, viewing, and locked purchase attempts do not transfer ownership. The preview ownership status now reads `Preview policy defined — no ownership transfer executed`; no buyer assignment, settlement, listing mutation, ownership transfer, or purchase execution has been enabled.
- Main 10 buyer game-copy readiness checkpoint: source readiness and payload readiness are separate derived gates; source readiness now shows `Yes`, payload readiness remains `No`, and combined buyer-game-copy readiness remains `No`. `COPY_BLOCKER_PAYLOAD_NOT_READY` is now the only active buyer game-copy blocker and continues to feed the readiness summary and Preview handoff plan summary.
- Main 10 buyer game-copy payload traceability checkpoint: all seven missing component IDs are derived from the structured component array and now flow through the component summary, buyer game-copy readiness summary, Preview purchase guard summary, top-level Buyer handoff readiness model, and Preview handoff plan summary; shared `renderPreviewIdListRow` rendering displays each ID on its own line while all purchase, ownership-transfer, persistence, and playable-handoff behavior remains disabled and preview-only.
- Main 10 top-level handoff readiness checkpoint: the Buyer handoff readiness model reads from the buyer game-copy readiness summary; combined handoff readiness requires both all six handoff requirements and buyer-game-copy readiness, currently shows `No`, exposes the remaining buyer-copy blocker ID `COPY_BLOCKER_PAYLOAD_NOT_READY`, and remains preview-only with purchase execution disabled.
...
- Main 10 engine quality checkpoint: the 85% coverage target is now cleared across every reported metric with `22` test files and `54` tests passing—statements `96.44%`, branches `87.17%`, functions `91.89%`, and lines `96.44%`; four orphaned move-wrapper modules and the direct test that existed only for one unused wrapper were removed after confirming the active registry and public API use `src/moves.ts`, and both the engine build and web production build pass after the cleanup.
- Main 10 undo checkpoint: sequential engine undos are now explicitly tested in reverse move order, with `22` test files and `55` tests passing; undo restores the playable board, score, tick, and history position while incrementing the undo count, but UI draw/recycle counters remain historical and consumed stock-card information value steps are not refunded.
- Main 10 economy value-step counter checkpoint: economy value consumption is now tracked separately from resettable UI draw/recycle counters. Draw 1 consumes `1` economy value step and Draw 3 consumes `3`; Undo does not refund consumed steps, and `Reset counters` resets the visible UI counters without restoring consumed economy value or increasing remaining/listing value. Starting a fresh game/session resets economy value-step consumption. Browser verification confirmed that after Draw 3 increased value steps from `3` to `6`, `Reset counters` returned the visible draw counter to `0` while value steps remained `6` and remaining value stayed reduced. Web production build and lint pass.
- Main 10 engine type-safety checkpoint: the legacy `MoveSpec` wrapper registry was replaced with one strongly typed dispatcher that translates the shared `DispatchMoveAction` union into engine moves; `MoveAction` now reuses that same contract across the public engine API and web app, the unused `src/moves/types.ts` layer and its obsolete coverage exclusion were removed, and active engine source now contains no explicit `any` usage. Engine build, web production build, and all `22` test files / `55` tests pass, with coverage at statements `95.94%`, branches `86.86%`, functions `93.54%`, and lines `95.94%`.
- Main 10 engine test type-safety checkpoint: unnecessary `any` casts and annotations were removed from the public API, dispatcher, history, move, and scoring tests; intentional invalid-action guard tests now pass malformed runtime actions through `unknown` into the typed action contract. All `22` test files / `55` tests pass, and coverage improved to statements `96.91%`, branches `88.00%`, functions `93.54%`, and lines `96.91%`.
