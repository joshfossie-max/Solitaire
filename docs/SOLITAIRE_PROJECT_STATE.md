...
- Main 10 buyer game-copy source traceability checkpoint: five structured source requirements now cover the listing metadata snapshot, playable game-state snapshot, buyer copy source, source freeze rule, and copy creation trigger; derived counts currently show `5` total, `1` complete, and `4` incomplete, with the four incomplete source requirement IDs flowing through the buyer game-copy readiness summary, Preview purchase guard summary, top-level Buyer handoff readiness model, and Preview handoff plan summary while all copy creation, purchase, ownership-transfer, persistence, and playable-handoff behavior remains disabled and preview-only.
